...
void RunMainMenu(void)
{
    ...
    //     icon_names[i] = (lesson_list_goldstars[i] ? "goldstar" :
    //     "no_goldstar");
    // T4K_CreateOneLevelMenu(MENU_LESSONS, num_lessons, lesson_list_titles,
    // icon_names, NULL, "Back"); T4K_PrerenderMenu(MENU_LESSONS);
...
static int chooseWordlist(void)
{
    ...
    /* First we add the bundled word lists in either NLS or default theme: */
    {
        if (!settings.use_english) /* Using theme: */
        {
            ...
            {
                fprintf(stderr,
                        "chooseWordList() - theme contains no wordlist dir \n");
                return 0;
            }
        }
        else /* No user settings or theme selected - using English: */
        {
            ...
            if (!T4K_CheckFile(wordPath))
            {
                fprintf(
                    stderr,
                    "chooseWordList() - data path contains no wordlist dir \n");
                return 0;
            }
        }
    }
    ...
    if (!T4K_CheckFile(wordPath))
    {
        // DEBUGMSG { fprintf(stderr, "chooseWordList() -  path \"%s\" not
        // found\n", wordPath); }
    }
    else
    {
        ...
        do
        {
            ...
            if (0 != strcmp(&wordsFile->d_name[strlen(wordsFile->d_name) - 4],
                            ".txt"))
            {
                continue;
            }
    ...
    {
        titles[i] =
            T4K_BlackOutline(wordlistName[i], DEFAULT_MENU_FONT_SIZE, &white);
        select[i] =
            T4K_BlackOutline(wordlistName[i], DEFAULT_MENU_FONT_SIZE, &yellow);
    }
    ...
    while (!stop)
    {
        ...
        while (SDL_PollEvent(&event))
        {
            ...
            {
            case SDL_EVENT_QUIT:
                exit(0); /* FIXME may need to cleanup memory and exit more
                            cleanly */
                break;
