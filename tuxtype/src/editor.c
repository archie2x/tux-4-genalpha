...
static void sort_wordlists(int n, char file_names[][FNLEN],
...
void ChooseListToEdit(void)
{
    ...
    char file_names[MAX_WORD_LISTS]
                   [FNLEN]; // names of the files containing lists
    char list_titles[MAX_WORD_LISTS]
                    [MAX_WORD_SIZE + 1]; // text list name to be displayed
    ...
    SDL_Surface* yellow_titles_surf[MAX_WORD_LISTS + 1] = {
        NULL}; // selected titles - yellow lettering
    // Rects where list names will be drawn on screen:
    static SDL_Rect titleRects[8];

    SDL_Surface* max_title_size =
        NULL; // using this to figure out size of wordlist name
    ...
    if (T4K_CheckFile(wordsDir))
    {
        DEBUGCODE
        {
            fprintf(stderr, "User specific wordlist path found: %s\n",
                    wordsDir);
        }
    }
    else
    ...
    if (!wordlist_files)
    {
        fprintf(
            stderr,
            "ChooseListToEdit() - cannot open custom word list directory!\n");
        return;
    }
    ...
    new_button =
        T4K_LoadImageOfBoundingBox("wordlist_button.png", IMG_ALPHA, 140, 50);
    remove_button =
        T4K_LoadImageOfBoundingBox("wordlist_button.png", IMG_ALPHA, 140, 50);
    done_button =
        T4K_LoadImageOfBoundingBox("wordlist_button.png", IMG_ALPHA, 140, 50);
    ...
    button_text_rect[New].x =
        button_rect[New].x + button_rect[New].w / 2 - NEW->w / 2;
    button_text_rect[New].y =
        button_rect[New].y + button_rect[New].h / 2 - NEW->h / 2;
    button_text_rect[Remove].x =
        button_rect[Remove].x + button_rect[Remove].w / 2 - REMOVE->w / 2;
    button_text_rect[Remove].y =
        button_rect[Remove].y + button_rect[Remove].h / 2 - REMOVE->h / 2;
    button_text_rect[Done].x =
        button_rect[Done].x + button_rect[Done].w / 2 - DONE->w / 2;
    button_text_rect[Done].y =
        button_rect[Done].y + button_rect[Done].h / 2 - DONE->h / 2;
    ...
    while (!stop)
    {
        ...
        if (change)
        {
            ...
            if (!reload_files)
            {
                LOG("ChooseListToEdit() - cannot open custom word list "
                    "directory!\n");
                return;
            }
            ...
            for (int j = 0; j < reload_count; j++)
            {
                ...
                if (T4K_CheckFile(fn) == 1)
                {
                    ...
                    list_titles[num_lists][0] =
                        toupper(list_titles[num_lists][0]);
                    fclose(fp);
                    strncpy(file_names[num_lists++], name, FNLEN - 1);
                }
            }
        ...
        if (old_loc != loc || redraw == 1)
        {
            ...
            for (i = 0; i < 5; i++)
            {
                SDL_BlitSurface(directions[i], NULL, screen,
                                &directions_Rect[i]);
            }
            ...
            for (i = start; i < MIN(start + 8, num_lists); i++)
            {
                ...
                if (i == loc)
                {
                    SDL_BlitSurface(yellow_titles_surf[loc], NULL, screen,
                                    &titleRects[i % 8]);
                }
                else
                {
                    SDL_BlitSurface(white_titles_surf[i], NULL, screen,
                                    &titleRects[i % 8]);
                }
                T4K_UpdateRect(screen, NULL);
            }
...
void EditWordList(char* words_file)
{
    ...
    else
    {
        DEBUGCODE
        {
            fprintf(stderr, "Editor: checking directory: %s/words",
    ...
    // NOTE: Works originally, but upon returning to editorlist, the word
    // selected
    ...
    if (fp)
    {
        ...
        while (fgets(words_in_list[number_of_words], MAX_WORD_SIZE, fp))
        {
            ...
            {
                fprintf(stderr,
                        "Read word \"%s\" from file, number_of_words = %d\n",
        ...
        while (SDL_PollEvent(&event))
        {
            ...
            case SDL_EVENT_KEY_DOWN:
            {
                        ...
                        if (number_of_words > 1)
                        {
                            ...
                            for (x = loc; x <= number_of_words - 1; x++)
                            {
                                if (x < number_of_words - 1)
                                {
                                    ...
                                    len = T4K_ConvertToUTF8(
                                        temp, words_in_list[x + 1],
                                        MAX_WORD_SIZE);
                ...
                if (i) // FIXME what is i?
                {
                    ...
                    if (len < MAX_WORD_SIZE - 1)
                    {
                        ...
                        temp[len + 1] = 0;
                        T4K_ConvertToUTF8(temp, words_in_list[loc + 1],
                                          MAX_WORD_SIZE);
            ...
            if (!stop)
            {
                ...
                for (i = start; i < MIN(start + 8, number_of_words - 1); i++)
                {
                    ...
                    if (i == loc)
                    {
                        SDL_BlitSurface(yellow_words[loc], NULL, screen,
                                        &word_rects[i % 8]);
                    }
                    else
                    {
                        SDL_BlitSurface(white_words[i], NULL, screen,
                                        &word_rects[i % 8]);
                    }
                }
                ...
                SDL_BlitSurface(wordlist_name, NULL, screen,
                                &wordlist_name_rect);
  ...
  word list*/
    ...
    else
    {
        ...
        {
            fprintf(stderr,
                    "In EditWordList(), unable to open %s for writing\n", fn);
        }
    }
...
int CreateNewWordList(void)
{
    ...
    // Creates a box thing, tells user to enter in name of list.  Click OK, or
    // CANCEL
     ...
     * CANCEL off the right edge for buttons wider than w/4.) */
    {
        ...
        OK_rect.x = ok_x;
        OK_rect.y = btn_y;
        OK_rect.w = OK_button->w;
        OK_rect.h = OK_button->h;
        ...
        CANCEL_rect.x = cnc_x;
        CANCEL_rect.y = btn_y;
        CANCEL_rect.w = CANCEL_button->w;
        CANCEL_rect.h = CANCEL_button->h;
        ...
        while (SDL_PollEvent(&event))
        {
            ...
            if (!stop)
            {
...

                {
                    ...
                    SDL_BlitSurface(CANCEL_button, NULL, screen, &CANCEL_rect);
                    CANCEL_rect_text.x =
                        cnc_x + CANCEL_button->w / 2 - CANCEL->w / 2;
                    CANCEL_rect_text.y = btn_y + (CANCEL->h / 2);
                    SDL_BlitSurface(CANCEL, NULL, screen, &CANCEL_rect_text);
                }
