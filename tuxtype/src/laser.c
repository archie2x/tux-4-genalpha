...
#define CITY_EXPL_START                                                        \
    (3 * 5) /* Must be mult. of 5 (number of expl frames)                      \
             */
#define COMET_EXPL_START                                                       \
    (2 * 2) /* Must be mult. of 2 (number of expl frames) */
...
static void laser_draw_line(int x1, int y1, int x2, int y2, int r, int g,
                            int b);
    ...
    do
    {
        ...
        for (; ans_num > 0; ans_num--)
        {
            ...
            if (lowest != -1)
            {
                ...
                laser_add_score((diff_level + 1) * 5 *
                                ((screen->h - comets[lowest].y) / 20));
            }
            else
            {
        ...
        if (tux_anim != -1)
        {
            ...
            {
                tux_img =
                    tux_anim + 1 - (tux_anim_frame / (ANIM_FRAME_START / 2));
            }
        }
        ...
        if (level_start_wait == 0 && (frame % 5) == 0 && gameover == 0)
        {
            if (num_attackers > 0)
            {
                ...
                if ((num_comets_alive < 2 || ((rand() % 4) == 0)) &&
                    distanceMoved > 40)
                {
                    distanceMoved = 0;
        ...
        if ((num_cities_alive == 0) && (gameover == 0))
        {
            ...
            T4K_Tts_say(DEFAULT_VALUE, DEFAULT_VALUE, INTERRUPT,
                        gettext("yep you miss it. hahh hahh haa. game over! "
                                "you scored %d goodbye!"),
                        score);
        }
        ...
        sprintf(str, "%d", wave);
        laser_draw_numbers(str,
                           images[IMG_WAVE]->w + (images[IMG_NUMBERS]->w / 10));
        ...
        for (i = 0; i < NUM_CITIES; i++)
        {
            ...
            img +=
                ((wave % MAX_CITY_COLORS) * (IMG_CITY_GREEN - IMG_CITY_BLUE));
            ...
            if (cities[i].shields)
            {
                ...
                dest.w = src.w;
                dest.h = src.h;
                SDL_BlitSurface(shield->frame[shield->cur], NULL, screen,
                                &dest);
            }
        }
        ...
        if (laser.alive)
        {
            laser_draw_line(laser.x1, laser.y1, laser.x2, laser.y2,
        ...
        if (paused)
        {
            ...
            if (quit == 0)
            {
                ...
                // Call announcer function in thread which annonces the word to
                // type
                if (settings.tts)
                {
                    tts_thread =
...
static void calc_city_pos(void)
{
    ...
    for (i = 0; i < NUM_CITIES; i++)
    {
        ...
        else
        {
            ...
            cities[i].x =
                i * screen->w / (NUM_CITIES) + images[IMG_CITY_BLUE]->w / 2;
        }
    }
}
...
static void laser_reset_level(int diff_level)
{
    ...
    DEBUGCODE
    {
        fprintf(stderr, "Will try to load file:\t%s", fname);
    }
...
static void laser_add_comet(int diff_level)
{
    ...
    DEBUGCODE
    {
        fprintf(stderr, "Adding %d comets \n", add);
    }
          ...
          */
    {
        ...
        if (!word || (wcslen(word) == 0) || (wcslen(word) > NUM_CITIES - 1))
        {
            fprintf(stderr, "Error - GetWord() returned NULL, zero-length, or "
                            "too-long word\n");
            return;
        }
        ...
        DEBUGCODE
        {
            fprintf(stderr, "word is: %S\tlength is: %d\n", word,
                    (int)wcslen(word));
        }
        do
        {
        ...
        for (i = 0; i < wcslen(word); i++)
        {
            ...
            if (location < MAX_COMETS)
            {
                ...
                DEBUGCODE
                {
                    fprintf(stderr, "Assigning letter to comet: %C\n", word[i]);
                }
            }
        }
    }
...
static void laser_draw_line(int x1, int y1, int x2, int y2, int red, int grn,
                            int blu)
{
    int      dx, dy, tmp;
    float    m, b;
    ...
    pixel = SDL_MapRGB(SDL_GetPixelFormatDetails(screen->format), NULL, red,
                       grn, blu);
    ...
    else
    {
        ...
        SDL_FillSurfaceRect(screen, &dest, pixel);
    }
}
...
static int tts_announcer(void* unused)
{
    ...
    while (1)
    {
        ...
        {
            T4K_Tts_say(DEFAULT_VALUE, DEFAULT_VALUE, INTERRUPT, "Score %d!",
                        score);
            SDL_Delay(20);
            T4K_Tts_wait();
            tts_announcer_switch = 1;
        ...
        {
            T4K_Tts_say(DEFAULT_VALUE, DEFAULT_VALUE, INTERRUPT,
                        "%d cities alive!", num_cities_alive);
            SDL_Delay(20);
            T4K_Tts_wait();
            tts_announcer_switch = 1;
        }
        ...
        for (i = 0; i < MAX_COMETS; i++)
        {
            if (comets[i].alive && comets[i].shootable && comets[i].expl == 0 &&
                comets[i].y > lowest_y)
            {
                lowest   = i;
                lowest_y = comets[i].y;
        ...
        if (1 < wcslen(comets[lowest].word))
        {
            for (i = comets[lowest].pos; i < wcslen(comets[lowest].word); i++)
            {
                ...
                buffer[iter] = L' ';
                iter++;
                buffer[iter] = comets[lowest].word[i];
                iter++;
            }
        }
        buffer[iter] = L'.';
        ...
        pitch_and_rate =
            ((lowest_y * 100) / (screen->h - images[IMG_CITY_BLUE]->h));
