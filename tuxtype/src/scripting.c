...
int XMLLesson(void)
{
    ...
    DEBUGCODE
    {
        fprintf(stderr, "script_path is: %s\n", script_path);
    }
    ...
    script_list_names = SDL_GlobDirectory(
        script_path, "*.xml", SDL_GLOB_CASEINSENSITIVE, &num_scripts);
    ...
    //
    //
    //      if (strcmp(&script_file->d_name[strlen(script_file->d_name) -
    //      4],".xml"))
    //        continue;
    //
    ...
    DEBUGCODE
    {
        fprintf(stderr, "Found %d . xml file(s) in script dir\n", num_scripts);
    }
    ...
    while (!stop)
    {
        while (SDL_PollEvent(&event))
        {
            ...
            {
            case SDL_EVENT_QUIT:
                return 0; /* Return control to the main program so we can exit
                             cleanly */
                break;
            ...
            case SDL_EVENT_MOUSE_BUTTON_DOWN:
                ...
                for (i = 0; (i < 8) && (loc - (loc % 8) + i < num_scripts); i++)
                {
                    if (inRect(titleRects[i], event.button.x, event.button.y))
                    {
                        ...
                        /* If braille is not enabled or scripts_braille folder
                         * does not exist then select the default path */

                        if (settings.use_english)
                        {
                            sprintf(fn, "%s/scripts_braille/%s",
            ...
            case SDL_EVENT_KEY_DOWN:
                ...
                {
                    /* If braille is not enabled or scripts_braille folder does
                     * not exist then select the default path */
        ...
        if (old_loc != loc)
        {
            ...
            for (i = start; i < MIN(start + 8, num_scripts); i++)
            {
                ...
                { /* Draw selected text in yellow:  */
                    SDL_BlitSurface(select[loc], NULL, screen,
                                    &titleRects[i % 8]);
                ...
                { /* Draw unselected text in white: */
                    SDL_BlitSurface(titles[i], NULL, screen,
                                    &titleRects[i % 8]);
                }
            }
    ...
    DEBUGCODE
    {
        fprintf(stderr, "Attempting to run script: %s\n", fn);
    }
...
static char* get_quote(const char* in)
{
    ...
    for (start = 0; start < strlen(in) && in[start] != '"'; start++)
        ; // find the first "

    if (start >= strlen(in))
    {
        return 0; // return null string if no " found
    }

    start++; // move past the "

    for (finish = start; finish < strlen(in) && in[finish] != '"'; finish++)
        ; // find the next "
...
static int load_script(const char* fn)
{
    ...
    do
    {
        ...
        if (strncmp("<!--", str, 4) == 0)
        {
            ...
            /* -- if we reached the end of the file and saw no close to the
             * comment, generate a warning -- */
            if (!found && fscanf_result == EOF)
            {
                fprintf(stderr, "XML Warning: End of file reached looking for "
        ...
        else if (strncmp("<page", str, 5) == 0)
        {
            ...
            {
                fprintf(stderr,
                        "CRITICAL XML ERROR: <page> should be in a <script> in "
                        "file %s line (todo)",
                        fn);
                close_script();
                return 0;
        ...
        else if (strncmp("<text", str, 5) == 0)
        {
            ...
            {
                fprintf(stderr,
                        "CRITICAL XML ERROR: <text> should be in a <page> in "
                        "file %s line (todo)",
                        fn);
                close_script();
                return 0;
            ...
            /* --- grab the text between <text> and </text> --- */
            {
                ...
                for (start = 5; start < strlen(str) - 5 && str[start] != '>';
                     start++)
                    ;
        ...
        else if (strncmp("<img", str, 4) == 0)
        {
            ...
            {
                fprintf(stderr,
                        "CRITICAL XML ERROR: <img> should be in a <page> in "
                        "file %s line (todo)",
                        fn);
                close_script();
        ...
        else if (strncmp("<bkgd", str, 5) == 0)
        {
            ...
            {
                fprintf(stderr,
                        "CRITICAL XML ERROR: <bkgd> should be in a <page> in "
                        "file %s line (todo)",
                        fn);
                close_script();
                return 0; // Return control to the main program for a clean exit
        ...
        else if (strncmp("<wav", str, 4) == 0)
        {
            ...
            {
                fprintf(stderr,
                        "CRITICAL XML ERROR: <wav> should be in a <page> in "
                        "file %s "
                        "line (todo)",
                        fn);
                close_script();
        ...
        else if (strncmp("<prac", str, 5) == 0)
        {
            ...
            {
                fprintf(stderr,
                        "CRITICAL XML ERROR: <prac> should be in a <page> in "
                        "file %s line (todo)",
            ...
            { /* --- grab the text between <prac> and </prac> --- */
                ...
                start++; // advance passed the '>/* --- grab the text between
                         // <prac> and </prac> --- */'
...
static void run_script(void)
{
    ...
    while (curPage)
    {
        ...
        {
            SDL_Surface* img =
                LoadImage(curPage->background, IMG_ALPHA | IMG_NOT_REQUIRED);

            /* hack: since this is the background it needs to scale when in
             * fullscreen but shouldn't every image scale when in fullscreen?
        ...
        {
            SDL_Surface* img =
                LoadImage(curScript->background, IMG_ALPHA | IMG_NOT_REQUIRED);
        ...
        while (curItem)
        {
            ...
            {
                SDL_Surface* img =
                    LoadImage(curItem->data, IMG_ALPHA | IMG_NOT_REQUIRED);
                if (img)
                {
            ...
            case itemBKGD:
            {
                ...
                if (img)
                {
                    /* hack: since this is the background it needs to scale when
                     * in fullscreen -MDT */
                    if (settings.fullscreen)
                    {
                        SDL_Surface* fsimg = zoom(img, fs_res_x, fs_res_y);
