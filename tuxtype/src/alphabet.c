...
/* An individual item in the list of cached unicode characters that are rendered
 * at   */
...

    {
        ...
        do
        {
            ...
            if ((wcslen(wide_str) >= 3) &&
                (wcstol(&wide_str[0], NULL, 0) >=
                 0) /* These lines just make sure the */
                && (wcstol(&wide_str[0], NULL, 0) <
                    10) /* finger is between 0 and 10     */
                && (wide_str[1] == '|') && (k < MAX_UNICODES) &&
                !unicode_in_key_list(
                    wide_str[2])) /* Make sure char not already added */
            {
                DEBUGCODE
                {
                    fprintf(stderr,
...
int GetFinger(int i)
{
    ...
    {
        fprintf(stderr,
                "GetFinger() - requested index i = %d outside of array\n", i);
        return -3;
    }
    ...
    if ((keyboard_list[i].finger < 0) || (keyboard_list[i].finger > 9))
    {
        fprintf(stderr,
                "GetFinger() - Unicode char '%C' has no valid finger.\n", i);
        return -1;
    }
...
int GetShift(int i)
{
    ...
    {
        fprintf(stderr,
                "GetShift() - requested index i = %d outside of array\n", i);
        return -1;
    }
...
int GetIndex(wchar_t uni_char)
{
    ...
    while ((i < MAX_UNICODES) && (keyboard_list[i].unicode_value != uni_char))
    {
        i++;
    }
...
wchar_t GetLastKey()
{
    ...
    if (!num_chars_used || num_chars_used < 0 || num_chars_used >= MAX_UNICODES)
    {
        return -1;
    }
    else
...
{
    static wchar_t last =
        -1; // we don't want to return same letter twice in a row
    wchar_t letter;
    int     i = 0;
...
wchar_t* GetWord(void)
{
    ...
    DEBUGCODE
    {
        fprintf(stderr, "Selected word is: %S\n", word_list[choice]);
    }
...
int GenerateWordList(const char* wordFn)
{
    ...
    DEBUGCODE
    {
        fprintf(stderr, "Loading words from file: %s\n", wordFn);
    }
    ...
    while (!feof(wordFile) && (num_words < MAX_NUM_WORDS))
    {
        ...
        // NOTE now the cascade and laser games have their own checks for how
        // long of words they can actually handle, so we let long words in now -
        // DSB

        //     if (length > MAX_WORD_SIZE)
        //     {
        //       fprintf(stderr, "Word '%s' not added - exceeds %d
        //       characters\n",
        ...
        if (num_words >= MAX_NUM_WORDS)
        {
            fprintf(
                stderr,
                "Word '%s' not added - list has reached max of %d characters\n",
        ...
        {
            fprintf(stderr,
                    "Word '%S' not added - contains Unicode chars not in "
                    "keyboard list\n",
                    temp_wide_word);
            continue;
        }
...
int RenderLetters(int font_size)
{
    ...
    while (i < MAX_UNICODES)
    {
        ...
        if (t[0] != 0)
        {
            ...
            {
                fprintf(stderr,
                        "Creating SDL_Surface for list element %d, char = "
                        "'%lc', Unicode value = %d\n",
                        i, *t, *t);
            }

            char_glyphs[j].unicode_value = t[0];
            char_glyphs[j].white_glyph =
                BlackOutline_w(t, font_size, &white, 1);
...
SDL_Surface* GetWhiteGlyph(wchar_t t)
{
    ...
    for (i = 0; (char_glyphs[i].unicode_value != t) && (i <= num_chars_used);
         i++)
    {
    }
    ...
    if (i > num_chars_used)
    {
        ...
        fprintf(stderr,
                "Could not find glyph for Unicode char '%C', value = %d\n", t,
                t);
        return NULL;
    }
...
SDL_Surface* GetRedGlyph(wchar_t t)
{
    ...
    for (i = 0; char_glyphs[i].unicode_value != t && i <= num_chars_used; i++)
    {
    }
...
int CheckNeededGlyphs(void)
{
    ...
    while ((i < MAX_UNICODES) && (char_list[i] != '\0'))
    {
        if (!GetWhiteGlyph(char_list[i]))
        {
            fprintf(stderr,
                    "\nCheckNeededGlyphs() - needed char '%C' (Unicode value = "
                    "%d) not found.\n",
                    char_list[i], char_list[i]);
            fprintf(stderr, "This probably means that the theme's "
                            "'keyboard.lst' file is incorrect or incomplete.\n"
                            "It could also mean that the available font lacks "
                            "needed characters\n");
            return 0;
        }
        i++;
...
int check_needed_unicodes_str(const wchar_t* s)
{
    ...
    while (i < MAX_WORD_SIZE && i < wcslen(s))
    {
        ...
        {
            fprintf(stderr,
                    "\ncheck_needed_unicodes_str() - needed char '%C' (Unicode "
                    "value = %d) not found.\n",
                    s[i], s[i]);
            return 0;
        }
...
int map_keys(wchar_t wide_char, kbd_char* keyboard_entry)
{
    ...
    {
    case '`':
        strcpy(keyboard_entry->keyname, "A00");
    ...
    case '~':
        ...
        break;
    case '1':
        strcpy(keyboard_entry->keyname, "A01");
        keyboard_entry->shift  = 0;
        keyboard_entry->finger = 0;
        break;
    case '!':
        ...
        break;
    case '2':
        strcpy(keyboard_entry->keyname, "A02");
        keyboard_entry->shift  = 0;
    ...
    case '@':
        ...
        break;
    case '3':
        strcpy(keyboard_entry->keyname, "A03");
        keyboard_entry->shift  = 0;
    ...
    case '#':
        ...
        break;
    case '4':
        strcpy(keyboard_entry->keyname, "A04");
        keyboard_entry->shift  = 0;
    ...
    case '$':
        ...
        break;
    case '5':
        strcpy(keyboard_entry->keyname, "A05");
        keyboard_entry->shift  = 0;
        keyboard_entry->finger = 3;
        break;
    case '%':
        ...
        break;
    case '6':
        strcpy(keyboard_entry->keyname, "A06");
        keyboard_entry->shift  = 0;
    ...
    case '^':
        ...
        break;
    case '7':
        strcpy(keyboard_entry->keyname, "A07");
        keyboard_entry->shift  = 0;
        keyboard_entry->finger = 6;
        break;
    case '&':
        ...
        break;
    case '8':
        strcpy(keyboard_entry->keyname, "A08");
        keyboard_entry->shift  = 0;
    ...
    case '*':
        ...
        break;
    case '9':
        strcpy(keyboard_entry->keyname, "A09");
        keyboard_entry->shift  = 0;
    ...
    case '(':
        ...
        break;
    case '0':
        strcpy(keyboard_entry->keyname, "A10");
        keyboard_entry->shift  = 0;
    ...
    case ')':
        ...
        break;
    case '-':
        strcpy(keyboard_entry->keyname, "A11");
        keyboard_entry->shift  = 0;
        keyboard_entry->finger = 9;
        break;
    case '_':
        ...
        break;
    case '=':
        strcpy(keyboard_entry->keyname, "A12");
        keyboard_entry->shift  = 0;
        keyboard_entry->finger = 9;
        break;
    case '+':
        ...
        break;
    case '\\':
        strcpy(keyboard_entry->keyname, "A13");
        keyboard_entry->shift  = 0;
        keyboard_entry->finger = 9;
        break;
    case '|':
        ...
        break;
    case 'q':
        strcpy(keyboard_entry->keyname, "B01");
        keyboard_entry->shift  = 0;
        keyboard_entry->finger = 0;
        break;
    case 'Q':
        ...
        break;
    case 'w':
        strcpy(keyboard_entry->keyname, "B02");
        keyboard_entry->shift  = 0;
        keyboard_entry->finger = 1;
        break;
    case 'W':
        ...
        break;
    case 'e':
        strcpy(keyboard_entry->keyname, "B03");
        keyboard_entry->shift  = 0;
        keyboard_entry->finger = 2;
        break;
    case 'E':
        ...
        break;
    case 'r':
        strcpy(keyboard_entry->keyname, "B04");
        keyboard_entry->shift  = 0;
        keyboard_entry->finger = 3;
        break;
    case 'R':
        ...
        break;
    case 't':
        strcpy(keyboard_entry->keyname, "B05");
        keyboard_entry->shift  = 0;
        keyboard_entry->finger = 3;
        break;
    case 'T':
        ...
        break;
    case 'y':
        strcpy(keyboard_entry->keyname, "B06");
        keyboard_entry->shift  = 0;
        keyboard_entry->finger = 6;
        break;
    case 'Y':
        ...
        break;
    case 'u':
        strcpy(keyboard_entry->keyname, "B07");
        keyboard_entry->shift  = 0;
        keyboard_entry->finger = 6;
        break;
    case 'U':
        ...
        break;
    case 'i':
        strcpy(keyboard_entry->keyname, "B08");
        keyboard_entry->shift  = 0;
        keyboard_entry->finger = 7;
        break;
    case 'I':
        ...
        break;
    case 'o':
        strcpy(keyboard_entry->keyname, "B09");
        keyboard_entry->shift  = 0;
        keyboard_entry->finger = 8;
        break;
    case 'O':
        ...
        break;
    case 'p':
        strcpy(keyboard_entry->keyname, "B10");
        keyboard_entry->shift  = 0;
        keyboard_entry->finger = 9;
        break;
    case 'P':
        ...
        break;
    case '[':
        strcpy(keyboard_entry->keyname, "B11");
        keyboard_entry->shift  = 0;
        keyboard_entry->finger = 9;
        break;
    case '{':
        ...
        break;
    case ']':
        strcpy(keyboard_entry->keyname, "B12");
        keyboard_entry->shift  = 0;
        keyboard_entry->finger = 9;
        break;
    case '}':
        ...
        break;
    case 'a':
        strcpy(keyboard_entry->keyname, "C01");
        keyboard_entry->shift  = 0;
        keyboard_entry->finger = 0;
        break;
    case 'A':
        ...
        break;
    case 's':
        strcpy(keyboard_entry->keyname, "C02");
        keyboard_entry->shift  = 0;
        keyboard_entry->finger = 1;
        break;
    case 'S':
        ...
        break;
    case 'd':
        strcpy(keyboard_entry->keyname, "C03");
        keyboard_entry->shift  = 0;
        keyboard_entry->finger = 2;
        break;
    case 'D':
        ...
        break;
    case 'f':
        strcpy(keyboard_entry->keyname, "C04");
        keyboard_entry->shift  = 0;
        keyboard_entry->finger = 3;
        break;
    case 'F':
        ...
        break;
    case 'g':
        strcpy(keyboard_entry->keyname, "C05");
        keyboard_entry->shift  = 0;
        keyboard_entry->finger = 3;
        break;
    case 'G':
        ...
        break;
    case 'h':
        strcpy(keyboard_entry->keyname, "C06");
        keyboard_entry->shift  = 0;
        keyboard_entry->finger = 6;
        break;
    case 'H':
        ...
        break;
    case 'j':
        strcpy(keyboard_entry->keyname, "C07");
        keyboard_entry->shift  = 0;
        keyboard_entry->finger = 6;
        break;
    case 'J':
        ...
        break;
    case 'k':
        strcpy(keyboard_entry->keyname, "C08");
        keyboard_entry->shift  = 0;
        keyboard_entry->finger = 7;
        break;
    case 'K':
        ...
        break;
    case 'l':
        strcpy(keyboard_entry->keyname, "C09");
        keyboard_entry->shift  = 0;
        keyboard_entry->finger = 8;
        break;
    case 'L':
        ...
        break;
    case ';':
        strcpy(keyboard_entry->keyname, "C10");
        keyboard_entry->shift  = 0;
        keyboard_entry->finger = 9;
        break;
    case ':':
        ...
        break;
    case '\'':
        strcpy(keyboard_entry->keyname, "C11");
        keyboard_entry->shift  = 0;
        keyboard_entry->finger = 9;
        break;
    case '"':
        ...
        break;
    case 'z':
        strcpy(keyboard_entry->keyname, "D02");
        keyboard_entry->shift  = 0;
        keyboard_entry->finger = 0;
        break;
    case 'Z':
        ...
        break;
    case 'x':
        strcpy(keyboard_entry->keyname, "D03");
        keyboard_entry->shift  = 0;
        keyboard_entry->finger = 1;
        break;
    case 'X':
        ...
        break;
    case 'c':
        strcpy(keyboard_entry->keyname, "D04");
        keyboard_entry->shift  = 0;
        keyboard_entry->finger = 2;
        break;
    case 'C':
        ...
        break;
    case 'v':
        strcpy(keyboard_entry->keyname, "D05");
        keyboard_entry->shift  = 0;
        keyboard_entry->finger = 3;
        break;
    case 'V':
        ...
        break;
    case 'b':
        strcpy(keyboard_entry->keyname, "D06");
        keyboard_entry->shift  = 0;
        keyboard_entry->finger = 3;
        break;
    case 'B':
        ...
        break;
    case 'n':
        strcpy(keyboard_entry->keyname, "D07");
        keyboard_entry->shift  = 0;
        keyboard_entry->finger = 6;
        break;
    case 'N':
        ...
        break;
    case 'm':
        strcpy(keyboard_entry->keyname, "D08");
        keyboard_entry->shift  = 0;
        keyboard_entry->finger = 6;
        break;
    case 'M':
        ...
        break;
    case ',':
        strcpy(keyboard_entry->keyname, "D09");
        keyboard_entry->shift  = 0;
        keyboard_entry->finger = 7;
        break;
    case '<':
        ...
        break;
    case '.':
        strcpy(keyboard_entry->keyname, "D010");
        keyboard_entry->shift  = 0;
        keyboard_entry->finger = 8;
        break;
    case '>':
        ...
        break;
    case '/':
        strcpy(keyboard_entry->keyname, "D11");
        keyboard_entry->shift  = 0;
        keyboard_entry->finger = 9;
        break;
    case '?':
        ...
        break;
    case ' ':
        strcpy(keyboard_entry->keyname, "E03");
        keyboard_entry->shift  = 0;
        keyboard_entry->finger = 5;
        break;
    // I think this is for when we call "keymap(-1, ...)"
    default:
        strcpy(keyboard_entry->keyname, "None");
        keyboard_entry->shift  = 0;
        keyboard_entry->finger = -1;
        break;
    }
...
void GenerateKeyboard(SDL_Surface* keyboard)
{
    ...
    DEBUGCODE
    {
        printf("Entering GenerateKeyboard\n");
    }
    ...
    if (!keyboard)
    {
        DEBUGCODE
        {
            printf("Error - keyboard SDL_Surface ptr null\n");
        }
        return;
    }
    ...
    DEBUGCODE
    {
        printf("Leaving GenerateKeyboard\n");
    }
}
...
{
    int i = 0;
    while ((char_list[i] != uc) && (char_list[i] != '\0') &&
    ...
    if (char_list[i] == '\0')
    {
        DEBUGCODE
        {
            fprintf(stderr, "Adding unicode value: %d\tcharacter %lc\n", uc,
                    uc);
        }
        char_list[i]     = uc;
        char_list[i + 1] = '\0';
