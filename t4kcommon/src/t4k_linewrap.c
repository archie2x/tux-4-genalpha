...
char wrapped_lines[MAX_LINES][MAX_LINEWIDTH]; // publicly available; declared
                                              // extern in t4k/common.h
                 ...
                 int width, int max_lines, int max_width)
{
    ...
    {
        fprintf(stderr,
                "warning: in linewrap(), length of input %d exceeds maximum "
                "%d, truncating\n",
    ...
    // mbs_width_linebreaks (input, length, width, 0, 0, NULL, locale_charset
    // (), breaks);
    ...
    for (strIndex = 0, i = 0; i < length; strIndex++, i++)
    {
        ...
        {
            str_list[listIndex][strIndex] =
                '\0';     // terminate the previous string
                      ...
                      int max_width)
{
    ...
    outputIndex = 0;
    for (inputIndex = 0;
         strlen(input[inputIndex]) > 0 && outputIndex < max_lines - 1;
         inputIndex++)
    {
        ...
        DEBUGMSG(debug_linewrap, "Not blank. Translated: %s\n",
                 _(input[inputIndex]));
        n_lines = T4K_LineWrap(_(input[inputIndex]), wrapped_lines0, width,
                               max_lines, max_width);
        ...
        {
            DEBUGMSG(debug_linewrap,
                     "intermedIndex %d, outputIndex %d, string %s\n",
                     intermedIndex, outputIndex, wrapped_lines0[intermedIndex]);
            strncpy(str_list[outputIndex], wrapped_lines0[intermedIndex],
                    max_width);
        }
    }
