...
/* menus is a global array when user can save up to 10 loaded menus.
   From outside this file we identify menu trees by their ids (indexes in this
   array) so as not to expose MenuNode struct to the user */
...
const float button_gap = 0.18, text_h_gap = 0.35, text_w_gap = 0.5,
            button_radius = 0.27;
...
MenuNode* create_empty_node()
{
    ...
    {
        DEBUGMSG(debug_menu,
                 "malloc(): Can't allocate memory for a new menu node.\n");
        return NULL;
    }
...
MenuNode* menu_LoadFile(char* file)
{
    ...
    if (menu == NULL)
    {
        DEBUGMSG(debug_menu_parser,
                 "menu_LoadFile: Failed to parse and load file. (`%s`)\n",
                 file);
        return NULL;
    }
                ...
                int (*handle_activity)(int, int))
{
    ...
    for (;;) /* one loop body execution for one menu page */
    {
        ...
        items                = min(menu->entries_per_screen,
                                   menu->submenu_size - menu->first_entry);
        ...
        for (i = 0; i < items; i++)
        {
            ...
            {
                SDL_BlitSurface(
                    menu_item_selected[i], NULL, T4K_GetScreen(),
                    &menu->submenu[menu->first_entry + i]->button_rect);
            }
            else
            {
                SDL_BlitSurface(
                    menu_item_unselected[i], NULL, T4K_GetScreen(),
                    &menu->submenu[menu->first_entry + i]->button_rect);
            }
            if (menu->submenu[menu->first_entry + i]->icon)
            {
                SDL_BlitSurface(
                    menu->submenu[menu->first_entry + i]->icon->default_img,
                    NULL, T4K_GetScreen(),
                    &menu->submenu[menu->first_entry + i]->icon_rect);
            }
        }
        ...
        if (menu->show_title)
        {
            ...
            title_surf =
                T4K_BlackOutline(_(menu->title), menu->font_size, &red);
            SDL_BlitSurface(title_surf, NULL, T4K_GetScreen(),
                            &menu_title_rect);
        ...
        while (!stop)
        {
            ...
            while (!stop && (SDL_PollEvent(&event) || first_loop))
            {
                switch (event.type)
                {
                    /* user decided to quit the application (for example by
                     * closing the window) */
                case SDL_EVENT_QUIT:
                {
                ...
                case SDL_EVENT_MOUSE_MOTION:
                {
                    ...
                    for (i = 0; i < items; i++)
                    {
                        if (T4K_inRect(menu->submenu[menu->first_entry + i]
                                           ->button_rect,
                    ...
                    /* Left/Right scroll arrows: hover sound on either if
                     * active. */
                    if ((T4K_inRect(prev_rect, event.motion.x,
                                    event.motion.y) &&
                         menu->first_entry > 0) ||
                        (T4K_inRect(next_rect, event.motion.x,
                                    event.motion.y) &&
                    ...
                    /* "stop" button */
                    else if (T4K_inRect(stop_rect, event.motion.x,
                                        event.motion.y))
                    {
                        if (click_flag)
                        {
                    ...
                    else // Mouse outside of arrow rects - re-enable click
                         // sound:
                    {
                        click_flag = 1;
                    }
                ...
                case SDL_EVENT_MOUSE_WHEEL:
                {
                    ...
                    else if (event.wheel.y < 0) /* wheel down */
                    {
                        ...
                        if (loc + 1 <
                            min(menu->submenu_size, menu->entries_per_screen))
                        {
                            loc++;
                        }
                        else if (menu->submenu_size <= menu->entries_per_screen)
                        ...
                        else if (menu->first_entry + menu->entries_per_screen <
                                 menu->submenu_size)
                        {
                            loc    = 0;
                            action = PAGEDOWN;
                ...
                case SDL_EVENT_MOUSE_BUTTON_DOWN:
                {
                    ...
                    else if (T4K_inRect(next_rect, event.motion.x,
                                        event.motion.y) &&
                             menu->first_entry + items < menu->submenu_size)
                    {
                        if (snd_click)
                    ...
                    else if (T4K_inRect(stop_rect, event.button.x,
                                        event.button.y))
                    {
                        if (snd_click)
                        {
                    ...
                    else if (loc != -1) // If focus is set from scrolling, use
                                        // click to activate
                    {
                        if (snd_click)
                        {
                ...
                case SDL_EVENT_KEY_DOWN:
                {
                    ...
                    case SDLK_J: // For grade-school Vim users
                    {
                        ...
                        if (loc + 1 < min(menu->submenu_size,
                                          menu->entries_per_screen) &&
                            loc + 1 < menu->submenu_size - menu->first_entry)
                        {
                            loc++;
                        }
                        else if (menu->submenu_size <= menu->entries_per_screen)
                    ...
                    case SDLK_TAB:
                    {
                        ...
                        else //[Shift] not pressed so go down:
                        {
                            ...
                            {
                                loc =
                                    0; // wrap around if only 1 T4K_GetScreen()
                            }
                            else if (menu->first_entry +
                                         menu->entries_per_screen <
                                     menu->submenu_size)
                            {
                                loc    = 0;
                        ...
                        //                else if(music_path)
                        //                {
                        //                  T4K_AudioMusicUnload(music_path,
                        //                  -1);
                        //                }
                        //                break;
                        //              }
           ...
           if handle_event() returns 1, menu should be redrawn */
                ...
                if (old_loc != loc || first_loop || event.key.key == SDLK_F10)
                {
                    DEBUGMSG(
                        debug_menu,
                        "run_menu(): changed button focus, old=%d, new=%d\n",
                        old_loc, loc);
                    ...
                    if (key == SDLK_F10)
                    {
                        T4K_PrerenderAll(); // Important when the screen is
                                            // being RESIZED
                        prerender_panel();  // To adjust the description panel
                                            // size with new resolution
                        if (loc == -1 && old_loc == -1)
                        {
                            loc = 0;
                        }
                    ...
                    {
                        tmp_rect = menu->submenu[old_loc + menu->first_entry]
                                       ->button_rect;
                        SDL_BlitSurface(menu_item_unselected[old_loc], NULL,
                                        T4K_GetScreen(), &tmp_rect);
                        if (menu->submenu[menu->first_entry + old_loc]->icon)
                        {
                            SDL_BlitSurface(
                                menu->submenu[menu->first_entry + old_loc]
                                    ->icon->default_img,
                                NULL, T4K_GetScreen(),
                                &menu->submenu[menu->first_entry + old_loc]
                                     ->icon_rect);
                    ...
                    if (loc + menu->first_entry >= 0 && loc <= items)
                    {
                        ...
                        if (menu->submenu[loc + menu->first_entry]->desc ==
                            NULL)
                        {
                            T4K_Tts_say(DEFAULT_VALUE, DEFAULT_VALUE, INTERRUPT,
                                        "%s",
                                        _(menu->submenu[loc + menu->first_entry]
                                              ->title));
                        }
                        else
                        {
                    ...
                    {
                        tmp_rect =
                            menu->submenu[loc + menu->first_entry]->button_rect;
                        SDL_BlitSurface(menu_item_selected[loc], NULL,
                                        T4K_GetScreen(), &tmp_rect);
                        if (menu->submenu[menu->first_entry + loc]->icon)
                        {
                            SDL_BlitSurface(
                                menu->submenu[menu->first_entry + loc]
                                    ->icon->default_img,
                                NULL, T4K_GetScreen(),
                                &menu->submenu[menu->first_entry + loc]
                                     ->icon_rect);
                            menu->submenu[menu->first_entry + loc]->icon->cur =
                                0;
                        }
                        T4K_UpdateRect(T4K_GetScreen(), NULL);
                        ...
                        // Set and render new description text
                        {
                            ...
                            char_width =
                                desc_chars_per_line(T4K_TOOLTIP_FONTSIZE);
                            T4K_LineWrapInsBreaks(desc, out, char_width, 64,
                                                  64);
                            //        desc_prerendered = T4K_SimpleText(out,
                            //        T4K_TOOLTIP_FONTSIZE, &yellow);
                            if (strcmp(desc, "") != 0)
                            {
                                desc_prerendered = T4K_BlackOutline(
                                    out, T4K_TOOLTIP_FONTSIZE, &yellow);
                            }
                        }
                        ...
                        {
                            SDL_Rect pos = {
                                T4K_GetScreen()->w * desc_panel_pos[0],
                                T4K_GetScreen()->h * desc_panel_pos[1]};
                            SDL_BlitSurface(desc_panel, NULL, T4K_GetScreen(),
                                            &pos);
                            SDL_BlitSurface(desc_prerendered, NULL,
                                            T4K_GetScreen(), &pos);
                            T4K_UpdateRect(T4K_GetScreen(), NULL);
                        }
                    }
                ...
                case CLICK:
                    ...
                    {
                        DEBUGMSG(debug_menu,
                                 "run_menu(): incorrect location for CLICK "
                                 "action (%d) !\n",
                                 loc);
                    }
                    else
                    {
                        ...
                        if (tmp_node->submenu_size == 0)
                        {
                            if (return_choice)
                            {
                                /* return choice instead of running a handler
                                 * function */
                                T4K_FreeSurfaceArray(menu_item_unselected,
                                                     items);
                                T4K_FreeSurfaceArray(menu_item_selected, items);
                            ...
                            else
                            {
                                ...
                                {
                                    /* we are going to run a handler function
                       that probably will run a game, save current screen
                       resolution in case it is changed while running a game */
                                    old_w = T4K_GetScreen()->w;
                                    old_h = T4K_GetScreen()->h;
                                    internal_res_switch_handler(NULL);
                                    if (handle_activity(tmp_node->activity,
                                                        tmp_node->param) ==
                                        QUIT)
                                    {
                                        /* user decided to quit while playing a
                                         * game */
                                        DEBUGMSG(debug_menu,
                                                 "run_menu(): "
                                                 "handle_activity() returned "
                                                 "QUIT message, exiting.\n");
                                        T4K_FreeSurfaceArray(
                                            menu_item_unselected, items);
                                        T4K_FreeSurfaceArray(menu_item_selected,
                                                             items);
                                        return QUIT;
                                    }
                                    if (old_w != T4K_GetScreen()->w ||
                                        old_h != T4K_GetScreen()->h)
                                    {
                                        T4K_PrerenderAll(); /* resolution has
                                                               changed */
                                    }
                                }
                            }
            ...
            if (stop)
            {
                /* Set and render new description text when menu is scrolled -
                 * Nalin */
                ...
                /* Announce the item again when page is scrolled or mouse
                 * hover*/
                if (loc >= 0 && loc < items)
                {
                ...
                /* whole menu will be redrawn so there is no need to draw
                 * anything now */
                break;
            }
            ...
            if (!stop && frame_counter % 5 == 0 && loc >= 0 && loc < items)
            {
                ...
                {
                    SDL_BlitSurface(
                        menu_item_selected[loc], NULL, T4K_GetScreen(),
                        &menu->submenu[menu->first_entry + loc]->icon_rect);
                    SDL_BlitSurface(
                        tmp_sprite->frame[tmp_sprite->cur], NULL,
                        T4K_GetScreen(),
                        &menu->submenu[menu->first_entry + loc]->icon_rect);
                    T4K_UpdateRect(
                        T4K_GetScreen(),
...
SDL_Surface** render_buttons(MenuNode* menu, bool selected)
{
    ...
    int           i;
    int           items =
        min(menu->entries_per_screen, menu->submenu_size - menu->first_entry);

    menu_items = (SDL_Surface**)malloc(items * sizeof(SDL_Surface*));
    if (NULL == menu_items)
    {
        DEBUGMSG(debug_menu,
                 "render_buttons(): failed to allocate memory for buttons!\n");
        return NULL; // error
    }
    ...
    for (i = 0; i < items; i++)
    {
        ...
        if (selected)
        {
            tmp_surf = T4K_CreateButton(curr_rect.w, curr_rect.h,
                                        button_radius * curr_rect.h, SEL_RGBA);
        }
        else
        {
            tmp_surf = T4K_CreateButton(curr_rect.w, curr_rect.h,
                                        button_radius * curr_rect.h, REG_RGBA);
        }
        ...
        /* text */
        tmp_surf =
            T4K_BlackOutline(_(menu->submenu[menu->first_entry + i]->title),
                             menu->font_size, selected ? &yellow : &white);
        SDL_BlitSurface(tmp_surf, NULL, menu_items[i],
                        &menu->submenu[menu->first_entry + i]->text_rect);
        SDL_DestroySurface(tmp_surf);
    }
...
void prerender_menu(MenuNode* menu)
{
    ...
    for (i = 0; i < menu->submenu_size; i++)
    {
        ...
        temp_surf =
            T4K_SimpleText(_(menu->submenu[i]->title), menu->font_size, &black);
        if (temp_surf)
        {
    ...
    button_w =
        max_text_w + ((found_icons ? 1.0 : 0.0) + 2.0 * text_w_gap) * button_h;
    ...
    for (i = 0; i < menu->submenu_size; i++)
    {
        ...
        curr_node->text_rect.x =
            ((found_icons ? 1.0 : 0.0) + text_w_gap) * curr_node->icon_rect.w;
        ...
        if (curr_node->icon_name)
        {
            ...
            DEBUGMSG(debug_menu,
                     "prerender_menu(): loading sprite %s for item #%d.\n",
                     filename, i);
            curr_node->icon = T4K_LoadSpriteOfBoundingBox(filename, IMG_ALPHA,
                                                          button_h, button_h);
        }
        if (!curr_node->icon_name ||
            !curr_node->icon) // if no sprite, or it failed to load
        {
...
char* find_longest_text(MenuNode* menu, int* length)
{
    ...
    {
        /*
       Use a small size (8) here, since text should be proportional and we're
       only interested in *which* text is longest, not its specific size
       */
...
int find_longest_menu_page(MenuNode* menu)
{
    ...
    {
        DEBUGMSG(debug_menu,
                 "find_longest_menu_page(): NULL pointer, exiting !\n");
        return 0;
    }
...
int binsearch(int min_f, int max_f, const char* text)
{
    ...
    {
        mid_f = (min_f + max_f) / 2;
        if (size_text(text, mid_f, &w, &h))
        {
            while (1)
                ;
        }
        if (w + (1.0 + 2.0 * text_w_gap) * (1.0 + 2.0 * text_h_gap) * h <
            menu_rect.w)
        {
            min_f = mid_f + 1;
        }
...
void T4K_SetMenuFontSize(MFStrategy strategy, int size)
{
    ...
    if (strategy != MF_EXACTLY && strategy != MF_UNIFORM &&
        strategy != MF_BESTFIT)
    {
        DEBUGMSG(debug_menu,
...
void T4K_PrerenderAll()
{
    ...
    stop_button = T4K_LoadImageOfBoundingBox(stop_path, IMG_ALPHA, stop_rect.w,
                                             stop_rect.h);
    ...
    prev_arrow = T4K_LoadImageOfBoundingBox(prev_path, IMG_ALPHA, prev_rect.w,
                                            prev_rect.h);
    if (prev_gray)
    {
        SDL_DestroySurface(prev_gray);
    }
    prev_gray = T4K_LoadImageOfBoundingBox(prev_gray_path, IMG_ALPHA,
                                           prev_rect.w, prev_rect.h);
    /* move button to the right */
    ...
    next_arrow = T4K_LoadImageOfBoundingBox(next_path, IMG_ALPHA, next_rect.w,
                                            next_rect.h);
    if (next_gray)
    {
        SDL_DestroySurface(next_gray);
    }
    next_gray = T4K_LoadImageOfBoundingBox(next_gray_path, IMG_ALPHA,
                                           next_rect.w, next_rect.h);
