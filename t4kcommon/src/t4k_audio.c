...
bool T4K_IsPlayingSound(Mix_Chunk* sound)
{
    ...
    for (int i = 0; i < SFX_TRACK_POOL; i++)
    {
        if (g_sfx_audio[i] == sound && g_sfx_tracks[i] &&
            MIX_TrackPlaying(g_sfx_tracks[i]))
        {
            return true;
        }
    }
...
{
    (void)
        channel; /* SDL3_mixer doesn't expose old channel ids; stop all SFX. */
    if (!g_mixer)
    {
        return;
    }
...
void T4K_AudioEnable(bool enabled)
{
    ...
    if (g_mixer)
    {
        MIX_SetMixerGain(g_mixer, enabled ? 1.0f : 0.0f);
    }
}
...
{
    if (!t4k_audio_ensure() || !g_music_track)
    {
        return;
    }
...
void T4K_SetSfxVolume(int volume)
{
    ...
    for (int i = 0; i < SFX_TRACK_POOL; i++)
    {
        if (g_sfx_tracks[i])
        {
            MIX_SetTrackGain(g_sfx_tracks[i], g);
        }
    }
}
